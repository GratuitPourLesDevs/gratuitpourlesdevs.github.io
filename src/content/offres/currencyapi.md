---
nom: "Currencyapi"
accroche: "API de conversion monétaire et de taux de change avec données courantes et historiques pour les projets personnels."
type: "API"
categorie: paiement-facturation
formule: "Free : 300 requêtes par mois"
inclus:
- "300 requêtes par mois"
- "10 requêtes par minute"
- "1 clé API"
- "Chiffrement HTTPS"
- "Taux actuels et historiques"
- "Mise à jour quotidienne"
conditions: []
restrictions:
- "Le plan Free est limité à un usage privé"
- "L’usage commercial et les mises à jour plus fréquentes nécessitent un plan payant"
alertes:
- type: fonctionnel
  niveau: important
  libelle: "Usage privé uniquement"
  detail: "Currencyapi réserve son plan gratuit de 300 requêtes mensuelles à un usage privé ; un projet commercial doit passer sur un plan payant."
usages: ["API", "Données", "Applications web"]
url: https://currencyapi.com/
documentation: https://currencyapi.com/docs
source: https://currencyapi.com/pricing/
couleur: "#10B981"
initiales: "CAP"
carteRequise: false
depassementFacture: false
permanent: true
statut: "active"
vedette: false
ordre: 1199
verifieLe: 2026-08-31
verificationAutomatiqueLe: 2026-08-31
verificationNote: "Source tarifaire accessible et mention d’une formule gratuite détectée."
verificationEtat: "confirmee"
verificationCode: "gratuit_detecte"
verificationEchecsConsecutifs: 0
---

Currencyapi est pratique pour ajouter une conversion de devises ou afficher des taux actuels et historiques dans un prototype sans maintenir sa propre source de données financières. La mise à jour quotidienne convient à des tableaux de bord, démonstrations et outils personnels où une précision à la minute n’est pas nécessaire. Le plan Free est cependant réservé à un usage privé : ses 300 requêtes mensuelles sont intéressantes pour développer et tester, mais un produit commercial doit passer sur une formule payante.
