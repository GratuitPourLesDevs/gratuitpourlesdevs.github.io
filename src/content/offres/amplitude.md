---
nom: "Amplitude"
accroche: "Plateforme d’analytics produit, session replay, expérimentation et activation avec un Free Tier très généreux."
type: "SaaS"
categorie: analytique-evenements-statistiques
formule: "Free : 2 millions d’événements par mois, sans limite de durée"
inclus:
- "2 000 000 événements par mois"
- "10 000 session replays par mois"
- "Analytics produit et agents IA"
- "Feature flags et expérimentation web avec limites"
- "Sièges utilisateurs illimités"
conditions: []
restrictions:
- "Les fonctions avancées et les volumes supérieurs nécessitent un plan payant"
- "Les guides, enquêtes et expérimentations sont limités sur Free"
alertes: []
usages: ["Données", "Applications web", "Mobile", "IA"]
url: https://amplitude.com/
source: https://amplitude.com/pricing
couleur: "#1E61F0"
initiales: "AMP"
carteRequise: false
depassementFacture: false
permanent: true
statut: "active"
vedette: false
ordre: 1155
verifieLe: 2026-08-31
verificationAutomatiqueLe: 2026-08-31
verificationNote: "Source tarifaire accessible et mention d’une formule gratuite détectée."
verificationEtat: "confirmee"
verificationCode: "gratuit_detecte"
verificationEchecsConsecutifs: 0
---

Amplitude devient particulièrement utile dès qu’une application ne veut plus seulement compter des visites, mais comprendre ce que font réellement ses utilisateurs : activation, funnels, rétention, parcours et abandons. Son volume gratuit permet d’instrumenter sérieusement un produit en phase de lancement, puis de compléter les événements avec du session replay sans multiplier les outils. C’est un excellent choix pour une équipe produit ou un SaaS qui veut prendre des décisions à partir des usages réels, en gardant à l’esprit que l’expérimentation et certaines fonctions avancées sont plus limitées sur le plan Free.
