---
nom: Tally
accroche: Constructeur de formulaires très généreux avec logique, paiements, signatures et intégrations.
type: SaaS
categorie: formulaires
formule: 'Free : formulaires et soumissions illimités sous fair use'
inclus:
- Formulaires illimités
- Soumissions illimitées
- Logique et calculs
- Paiements
- Signatures
- Téléversements
- Webhooks
- Google Sheets, Notion, Airtable, Zapier et Make
conditions: []
restrictions:
- Soumis aux règles de fair use
- Le retrait du branding et certaines fonctions d’équipe sont payants
alertes: []
usages:
- Applications web
- Engagement utilisateur
- Automatisation
url: https://tally.so/
source: https://tally.so/pricing
couleur: '#2563EB'
initiales: T
carteRequise: false
depassementFacture: false
permanent: true
statut: active
vedette: false
ordre: 770
verifieLe: '2026-08-20'
verificationAutomatiqueLe: '2026-08-20'
verificationNote: Offre gratuite vérifiée le 2026-08-20.
---

Tally — Constructeur de formulaires très généreux avec logique, paiements, signatures et intégrations. Free : formulaires et soumissions illimités sous fair use. Les limites et conditions ci-dessus reflètent l’offre vérifiée le 20 août 2026 et privilégient les informations actuellement publiées par le service.
