...
Cloudflare Pages est un bon choix pour publier une documentation, un portfolio, une landing page ou un frontend généré statiquement depuis GitHub, GitLab ou Wrangler. Chaque commit peut produire une prévisualisation partageable, ce qui facilite la validation d’une modification avant son passage en production, tandis que le CDN et le certificat HTTPS sont gérés par Cloudflare. Le plan gratuit ne facture pas les requêtes ni la bande passante des fichiers statiques et accepte les domaines personnalisés : un petit site public peut donc rester durablement en ligne sans budget d’hébergement. La limite apparaît lorsqu’on ajoute du calcul dynamique, car Pages Functions consomme alors les quotas de Cloudflare Workers. Une application avec beaucoup d’exécutions serveur, des traitements longs ou des besoins d’isolation avancés doit donc être dimensionnée comme un projet Workers plutôt que comme un simple site statique.
