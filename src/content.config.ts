...
const offres = defineCollection({
  ...
  schema: z.object({
    ...
    verificationEtat: z.enum(['confirmee', 'inconclusive', 'controle_requis']).optional(),
    verificationCode: z.enum(['gratuit_detecte', 'gratuit_non_detecte', 'site_indisponible', 'source_indisponible', 'controle_bloque']).optional(),
    verificationEchecsConsecutifs: z.number().int().nonnegative().default(0),
